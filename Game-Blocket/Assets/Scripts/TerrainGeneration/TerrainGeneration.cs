...
public class TerrainGeneration : NetworkBehaviour {
	...
	public void FixedUpdate() {
		if(chunksUpdatedLastCheck == null || 
			Vector3.Distance(GlobalVariables.PlayerPos, chunksUpdatedLastCheck) >= (GlobalVariables.WorldData.ChunkDistance* GlobalVariables.WorldData.ChunkWidth)/8)
			UpdateChunks();
	}
	...
	public void CheckChunksAroundPlayer(Vector3 playerPos) {
		...
		lock (ChunksVisibleLastUpdate) { 
			...
			{
				float f = Vector2.Distance(chunkGo.transform.position, playerPos);
