         ...
         for (int x = 0; x < World.ChunkWidth; x++)
         {
             ...
             for (int y = world.ChunkHeight - 1; y >= 0; y--)
             {
                 ...
                 if (heightvalue == World.Biom[(int)biomNoiseMap[x, y]].Regions[blockIDpos].RegionRange)
                 {
                     ...
                     heightvalue++;
                 if (heightvalueBG == World.Biom[(int)biomNoiseMap[x, y]].BgRegions[blockIDposBG].RegionRange)
                 {
                     blockIDposBG++;
                     heightvalueBG = 0;
                 }
                 ...
                 {
                     BlockIDs[x, y] = World.Biom[(int)biomNoiseMap[x, y]].Regions[blockIDpos].BlockID;
                 ...
                 BlockIDsBG1[x, y] = World.Biom[(int)biomNoiseMap[x, y]].BgRegions[blockIDposBG].BlockID;
             }
         }
         PlaceTiles(biomNoiseMap, true);
     }*/
    public void GenerateChunk(float[] noisemap, float[,] caveNoisepmap, float[,] biomNoiseMap)
    {
        for (int x = 0; x < World.ChunkWidth; x++)
