	...
	private class Command {
		...
		public static List<Command> Commands { get; } = new List<Command>()
		{
			...
			new Command("summon"){ action = (str) => { 
				// /summon 1 ~ ~ ~
				List<string> arguments = new List<string>(CutCmd(str).Split(' '));
				arguments.RemoveAt(0);
				try{
					//Enemy id
					uint enemyId = uint.Parse(arguments[0]);
					//X
					float x = arguments[1].StartsWith("~") ? GlobalVariables.LocalPlayerPos.x + float.Parse(arguments[1].Substring(1)) : float.Parse(arguments[1]);
                    //Y
                    float y = arguments[2].StartsWith("~") ? GlobalVariables.LocalPlayerPos.x + float.Parse(arguments[2].Substring(1)) : float.Parse(arguments[2]);
					MobHandler.Singleton.SpawnMob(enemyId, new Vector3(x, y, 0));
				}catch(Exception e){
					PrintToChat(str: e.ToString());
				}
			}, Synms = new List<string>(){"sum"} },
